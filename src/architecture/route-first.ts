...
import { generateNodeFormGeometry } from './node-form-geometry';
 ...
 * world-space lattice volume in the production runtime. Node geometry is now
 * selected by the same formId that owns hold/blocker behaviour, so the visual
 * machine and traversal grammar are one contract.
 */
export function generateRouteFirstArchitecture(
  ...
  return {
    ...
    interactions,
    pieces: generateNodeFormGeometry(world, interactions),
  };
}
